...
store served an ordinary datacenter address on every route with no key, no
proxy and no User-Agent header at all. If a plain run starts coming back
blocked, the site has started gating its catalogue and the README's central
claim needs re-measuring the same day.
...
- **A product document is not a small listing, and each parser must REFUSE
  the other's payload.** They are different documents — `{"products": [...]}`
  against `{"product": {...}}` — and a listing parser handed a product would
  otherwise emit one row whose `position` is 1 and whose page arithmetic was
  invented, so the run would look complete. Both directions raise, on the
  document's own evidence, and `smoke_test.py` pins both.

- **`/products/{h}.js` prices in INTEGER CENTS** and every other route in a
  decimal string. Which one a payload is is detected from the payload rather
  than from the URL, because a `--dump-html` replay arrives without one, and
  reading either as the other is a factor of 100 in every price.

- **Options are not positional.** `option1`/`option2` are Color and Size on
  622 of 665 products, and the store also ships `Rate`, `Length`,
  `Quantity`, `SPF`, `Pack Size`, `Strength` and `Product Size`. Read them by
  the product's own option NAME, or a rate band lands in the size column.
...
- **`hcaptcha` is not a block marker.** This store ships Shopify's
  storefront-forms hCaptcha bundle on **every page it serves**, bound to form
  submits, so a bare `hcaptcha` marker would report every successful run as a
  block — the exact mistake a sibling repo shipped with `akamai`. Neither is
  **`cf-turnstile`**, which is the
...
andieswim.com, say in the PR what you ran, which mode and URL, from which
exit, and what you got — including the sidecar's `status`, `stop_reason`,
`locale`, `currency`, `currency_source` and `catalog_count`, and the coverage
lines the run prints.

Three things about running this live that are specific to this store:

* **No mode needs a special exit.** All of them answered a plain datacenter
  address on 2026-09-18, so "it worked from my laptop" is reproducible here
...
* **Say which MARKET you ran.** Prices are set per market, not converted, so
  ...
  variant: 112.00 USD, 195.00 CAD, 110.00 GBP, 175.00 AUD, 130.00 EUR,
  21600 JPY.
* **If you compare against a page you opened in a browser, expect the market
  to differ.** There is no HTTP redirect on this store, but the theme's
  client-side geolocation app moves a real browser to your own market after
  load. The JSON endpoints this repo reads do not run JavaScript and do not
  move, which is why they are what it reads.
...
Do not add anything that submits a form or puts anything in a cart. This
store's pages carry an add-to-cart flow, an account area and a returns portal;
this project reads the catalogue and must never touch any of them. The site's
own `robots.txt` asks agents not to complete a checkout automatically, and
this tool does not.

Note that a form submit is also the ONLY place the store's captcha lives (see
the README), so "do not submit forms" and "never meets a captcha" are the
same rule stated twice.

## Scope

This repo scrapes **public catalogue data** on andieswim.com: collection
listings, search results and product documents, exactly as an anonymous
visitor is served them, and via the store's own public JSON endpoints.

Out of scope: anything behind a login, anything that submits a form
(including the cart, the account area and the newsletter), anything that
completes or approaches a checkout, and anything that defeats a protection
rather than passing it the way an ordinary browser does.

Also deliberately out of scope: **customer reviews**. The store carries them
through a third party, and republishing a named person's words is a separate
act from the store showing them on its own page (§10). There is no column for
them and adding one is a product decision rather than a bug fix.
